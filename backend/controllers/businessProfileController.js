...
export async function createBusinessProfile(req, res) {
  try {
    ...
    if (!userId) {
      return res
        .status(401)
        .json({ sucess: false, message: "Authentication required." });
    }
    ...
    console.error("CreateBusiness Profile error.", error);
...
export async function updateBusinessProfile(req, res) {
  try {
    ...
    console.error("UpdateBusiness Profile error.", error);
    return res.status(500).json({
      success: false,
      message: "Server Error",
    });
...
export async function getMyBusinessProfile(req, res) {
  try {
    ...
    console.error("GetBusiness Profile error.", error);
    return res.status(500).json({
      success: false,
      message: "Server Error",
    });
